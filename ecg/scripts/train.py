...
from models.lightning_module import MultiTaskClassifier, EncoderUnfreezeCallback
...
def create_callbacks(config: dict, checkpoint_dir: Path) -> list:
    ...
    callbacks.append(EncoderUnfreezeCallback(freeze_epochs=10, encoder_lr=1e-5))
...
def main():
    ...
    augmentation_config = AugmentationConfig(
        ...
        crop_prob=aug_params.get('crop_prob', 0.0),           # ← ADD
        min_crop_seconds=aug_params.get('min_crop_seconds', 3.0),  # ← ADD
    )
    ...
    base_model = create_hubert_multitask(
        ...
        hubert_size=model_config.get("hubert_size", "base"),
        dropout=model_config.get("dropout", 0.1),
    )    

    print(f"✓ Model: {model_config['architecture']} ({model_config['hubert_size']})")
