...
def _load_hubert_encoder(size: str) -> nn.Module:
    ...
    # Keep the FULL model including the CNN feature extractor so that passing
    # input_values=(batch, time) works correctly through model.forward().
...
class HuBERTECGMultiTask(nn.Module):
    ...
    def _encode(self, x: torch.Tensor) -> torch.Tensor:
        ...
        HuBERT-ECG expects input_values of shape (batch, time) — no lead dim.
        We treat each lead as an independent batch element:
          (B, 12, W) → (B*12, W) → encoder(input_values=...) → pool time
                     → (B*12, d_model) → reshape (B, 12, d_model)
                     → mean leads → (B, d_model)
        """
        B, n_leads, W = x.shape

        # Flatten leads into batch dimension: (B, 12, W) → (B*12, W)
        x_flat = x.reshape(B * n_leads, W)

        # Use input_values= kwarg to route through the full HubertModel.forward()
        # which runs CNN feature extraction + transformer in one call.
        out = self.encoder(input_values=x_flat)

        # Unpack output dataclass
