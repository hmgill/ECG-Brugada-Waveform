...
def build_model_and_load_checkpoint(
    ...
    """
    Reconstruct the base model and load weights from checkpoint.

    MultiTaskClassifier excludes loss_fn from save_hyperparameters, so
    Lightning cannot restore it automatically. We build a dummy loss and
    pass both model + loss to load_from_checkpoint, exactly as train.py does
    when constructing the module before training.
    """
    mc = cfg["model"]
