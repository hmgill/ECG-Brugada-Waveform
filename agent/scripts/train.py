...
Usage:
    python scripts_unified/train.py --config config_unified/train.yaml
"""
...
class BestModelMetricsCallback(Callback):
    ...
    def on_train_end(self, trainer, pl_module):
        ...
        # Log to MLflow — prefixed so they're easy to filter
        mlflow.log_metrics(
            {f"best/{k}": v for k, v in self.best_metrics.items()
             if isinstance(v, (int, float))}
        )

        # Log to Logfire as a single structured event
        logfire.info(
            'Best model metrics',
...
def parse_args():
    ...
    parser.add_argument(
        '--config',
        type=Path,
        default=Path('config_unified/train.yaml'),
...
def main():
    ...
    print(f"  Brugada pos_weight: {class_weights['brugada_pos_weight']:.3f}")
    ...
    base_model = create_ecg_transformer_rope(
        ...
        num_subclasses=model_config['num_subclasses'],
    ...
    if loss_type == 'weighted_bce':
        loss_kwargs = {
            ...
            'brugada_pos_weight': class_weights['brugada_pos_weight']
        }
        ...
        loss_kwargs = {
            ...
            'alpha_brugada': focal_cfg.get('alpha_brugada', 0.25),
    ...
    # Create callbacks
    checkpoint_dir = Path('checkpoints_unified') / run_name
    ...
    trainer = pl.Trainer(
        ...
        strategy=config['hardware'].get('strategy', 'auto'),  # DDP for multi-GPU
        callbacks=callbacks,
        logger=logger,
