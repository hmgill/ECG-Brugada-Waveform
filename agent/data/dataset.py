...
class UnifiedECGDataset(Dataset):
    ...
    def _bandpass_filter(self, signal: np.ndarray) -> np.ndarray:
        """
        Zero-phase clinical ECG bandpass filter (0.5–40Hz, 4th order Butterworth).
        - 0.5Hz high-pass removes baseline wander
        - 40Hz low-pass removes EMG/powerline noise
        - filtfilt ensures zero phase shift (no ST morphology distortion)
        
        Applied after resampling so filter always runs at target_sampling_rate.
        ...
        # Safety check: high must be < 1.0 (i.e. target_sampling_rate > 80Hz)
        if high >= 1.0:
            return signal  # Can't filter — sampling rate too low

        b, a = butter(4, [low, high], btype='band')
    
        # filtfilt needs signal longer than padlen (~18 samples for order 4)
        # Zero-signal fallback arrays are safe since they're all zeros
        if signal.shape[0] < 27:
            return signal
        
        return filtfilt(b, a, signal, axis=0)  # axis=0 = time dimension
